...
def test_deleted_basket_breaks_register_formula(engine, tsh):
    a = pd.Series(
        [1, 2, 3],
        index=pd.date_range(utcdt(2026, 1, 1), periods=3, freq='D')
    )
    tsh.update(engine, a, 'ghost-basket-series-A', 'Babar')
    tsh.update(engine, a, 'unrelated-series', 'Babar')

    tsh.register_basket(
        engine,
        'ghost-basket',
        '(by.name "ghost-basket-series")'
    )

    tsh.register_formula(
        engine,
        'formula-with-ghost-basket',
        '(add (findseries (by.basket "ghost-basket")))'
    )
...
def test_start_of_month(engine, tsh):
    i = Interpreter(engine, tsh, {})
    a = lisp.evaluate('(start-of-month (date "1973-05-20 09:01:42"))', i.env)
    assert a == pd.Timestamp('1973-05-01 00:00:00+0000', tz='UTC')
...
2023-06-01 08:00:00+00:00    2.666667
2023-06-01 09:00:00+00:00    3.000000
2023-06-01 10:00:00+00:00    3.333333
2023-06-01 11:00:00+00:00    3.666667
2023-06-01 12:00:00+00:00    4.000000
""", ts)
    ...
    ts = tsh.get(
        ...
        '3hourly_interpolated',
        from_value_date=pd.Timestamp('2023-06-01 01:00'),
        to_value_date=pd.Timestamp('2023-06-01 06:00')
    )
